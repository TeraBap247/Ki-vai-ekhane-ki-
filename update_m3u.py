...
def find_all_xtream_matches(tpl_name, streams, category_map, min_ratio=0.5):
    """
    ...
    for ch in streams:
        ...
        # BDIX / CDN detect more flexibly
        if "bdix" in cat_name:
            group = 0
        elif "cdn" in cat_name:
            group = 1
        else:
            # BDIX/CDN chara onno category diye akhon duplicate banacchi na
            continue
    ...
    while i < n:
        if lines[i].startswith("#EXTINF:"):
            ...
            if block:
                ...
                # Ignore list er channel hole untouched (duplicate o banabo na)
